...
def makeCatTemplates(datahists,sighists,bkghists,discriminant):
	...
	for BRind in range(nBRconf):
			...
			for cat in catList:
				...
				for proc in bkgProcList+bkgGrupList:
					...
					if proc in ttbarGrupList+ttbarProcList:
						for plotKey in ['JetPtVsAbsEta', 'JetCountInPDG', 'EventCount', 'TopBDR', 'TopBEta',
						                'recoTopBEta', 'TopBPhi', 'recoTopBPhi', 'TopBPt', 'recoTopBPt', 'TopBPt2D',
						                'JetCount2d', 'WeightProd','AK4HT', 'Eta']:
							hists[proc + denum + i + plotKey] = bkghists[plotKey + '_' + lumiStr + '_' + cat + '_' + bkgProcs[proc][0]+denum].Clone(plotKey + '_' + lumiStr + '_' + cat + '__' + proc + denum)
						...
						for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
							try:
								hists[proc + denum + i+'Bin'+bcTruthBinInx] = bkghists['Bin'+bcTruthBinInx+histoPrefix + '_' + bkgProcs[proc][0] + denum].Clone('Bin'+bcTruthBinInx+histoPrefix + '__' + proc + denum)
							except:
								print " b-c Bin hists not read in or does not exist for key:" + 'Bin'+bcTruthBinInx+histoPrefix + '_' + bkgProcs[proc][0] + denum
								pass
							for plotKey in ['AK4HT', 'Eta']:
								try:
									hists[proc + denum + i +plotKey+ 'Bin' + bcTruthBinInx] = bkghists['Bin' + bcTruthBinInx + plotKey + '_' + lumiStr + '_' + cat + '_' + bkgProcs[proc][0] + denum].Clone('Bin' + bcTruthBinInx + plotKey + '_' + lumiStr + '_' + cat + '__' + proc + denum)
								except:
									print " b-c Bin hists not read in or does not exist for key:" + 'Bin' + bcTruthBinInx + plotKey + '_' + lumiStr + '_' + cat + + '_' + \
									      bkgProcs[proc][0] + denum
									pass
					...
					for bkg in bkgProcs[proc]:
						if bkg!=bkgProcs[proc][0]:
							...
							if proc in ttbarGrupList+ttbarProcList:
								for plotKey in ['JetPtVsAbsEta', 'JetCountInPDG', 'EventCount', 'TopBDR', 'TopBEta',
								                'recoTopBEta', 'TopBPhi', 'recoTopBPhi', 'TopBPt', 'recoTopBPt',
								                'TopBPt2D',
								                'JetCount2d', 'WeightProd','AK4HT', 'Eta']:
									hists[proc + denum+i+plotKey].Add(bkghists[plotKey+'_'+lumiStr+'_'+cat+'_'+bkg+ denum])
								for flavType in ['Bflav', 'topBflav', 'Cflav', 'LiFlav']:
									try:
								...
								for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
									try:
										hists[proc + denum + i + 'Bin' + bcTruthBinInx].Add(bkghists['Bin' + bcTruthBinInx+histoPrefix + '_' + bkg+denum])
									except:
										print " b-c Bin hists not read in or does not exist for key:" +'Bin' + bcTruthBinInx+histoPrefix + '_' + bkg+denum
										pass
									for plotKey in ['AK4HT', 'Eta']:
										try:
											hists[proc + denum + i+plotKey + 'Bin' + bcTruthBinInx].Add(bkghists['Bin' + bcTruthBinInx + plotKey + '_' + lumiStr + '_' + cat + '_' + bkg + denum])
										except:
											print " b-c Bin hists not read in or does not exist for key:" + 'Bin' + bcTruthBinInx + plotKey + '_' + lumiStr + '_' + cat + '_' + bkg + denum
											pass
					...
					for bcuts in bcutlist:
						hists[proc + denum + i+bcuts] = bkghists[bcuts+histoPrefix + '_' + bkgProcs[proc][0] + denum].Clone(bcuts+histoPrefix + '__' + proc + denum)
						if proc in ttbarGrupList + ttbarProcList:
							for plotKey in ['JetPtVsAbsEta','WeightProd','AK4HT', 'Eta']:
								hists[proc + denum + i + plotKey + bcuts] = bkghists[bcuts + plotKey +'_' + lumiStr + '_' + cat + '_' + bkgProcs[proc][0] + denum].Clone(bcuts + plotKey +'_' + lumiStr + '_' + cat + '__' + proc + denum)
							# hists[proc + denum + i + "WeightProd"+bcuts] = bkghists[bcuts+'WeightProd_' + lumiStr + '_' + cat + '_' + bkgProcs[proc][0] + denum].Clone(bcuts+'WeightProd_' + lumiStr + '_' + cat + '__' + proc + denum)
							# hists[proc + denum + i + "JetPtVsAbsEta"+bcuts] = bkghists[bcuts+'JetPtVsAbsEta_' + lumiStr + '_' + cat + '_' + bkgProcs[proc][0] + denum].Clone(bcuts+'JetPtVsAbsEta_' + lumiStr + '_' + cat + '__' + proc + denum)
							for flavType in ['Bflav', 'topBflav', 'Cflav', 'LiFlav']:
								try:
									hists[proc + denum + i + flavType+bcuts] = bkghists[bcuts+histoPrefix + '_' + bkgProcs[proc][0] + denum + flavType].Clone(bcuts+histoPrefix + '__' + proc + denum + flavType)
								except:
									print flavType + " flavour hists not read in or does not exist in "+denum+'_'+bcuts+proc
									pass
							for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
								try:
									hists[proc + denum + i + 'Bin' + bcTruthBinInx+bcuts]=bkghists[bcuts+ 'Bin' + bcTruthBinInx+histoPrefix + '_' + bkgProcs[proc][0] + denum].Clone(bcuts+ 'Bin' + bcTruthBinInx+histoPrefix + '__' + proc + denum)
								except:
									print " b-c Bin hists not read in or does not exist for key:"+ bcuts+ 'Bin' + bcTruthBinInx+histoPrefix + '_' + bkgProcs[proc][0] + denum
									pass
								for plotKey in ['AK4HT', 'Eta']:
									try:
										hists[proc + denum + i +plotKey+ 'Bin' + bcTruthBinInx + bcuts] = bkghists[bcuts + 'Bin' + bcTruthBinInx + plotKey +'_' + lumiStr + '_' + cat + '_' + bkgProcs[proc][0] + denum].Clone(bcuts + 'Bin' + bcTruthBinInx + plotKey +'_' + lumiStr + '_' + cat + '__' + proc + denum)
									except:
										print " b-c Bin hists not read in or does not exist for key:" + bcuts + 'Bin' + bcTruthBinInx + plotKey +'_' + lumiStr + '_' + cat + '_' + bkgProcs[proc][0] + denum
										pass

						for bkg in bkgProcs[proc]:
							if bkg!=bkgProcs[proc][0]:
								hists[proc + denum + i+bcuts].Add(bkghists[bcuts+histoPrefix+'_'+bkg+ denum])
								if proc in ttbarGrupList+ttbarProcList:
									for plotKey in ['JetPtVsAbsEta','WeightProd', 'AK4HT', 'Eta']:
										hists[proc + denum + i + plotKey + bcuts].Add(bkghists[bcuts + plotKey+ '_' + lumiStr + '_' + cat + '_' + bkg + denum])
									# hists[proc + denum+i+"WeightProd"+bcuts].Add(bkghists[bcuts+'WeightProd_'+lumiStr+'_'+cat+'_'+bkg+ denum])
									# hists[proc + denum+i+"JetPtVsAbsEta"+bcuts].Add(bkghists[bcuts+'JetPtVsAbsEta_'+lumiStr+'_'+cat+'_'+bkg+ denum])
									for flavType in ['Bflav', 'topBflav', 'Cflav', 'LiFlav']:
										try:
											hists[proc + denum+i+flavType+bcuts].Add(bkghists[bcuts+histoPrefix + '_' + bkg+denum+flavType])
										except:
											print flavType + " flavour hists not read inor does not exist in " +denum+'_'+bcuts+bkg
											pass
									for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
										try:
											hists[proc + denum + i + 'Bin' + bcTruthBinInx + bcuts].Add(bkghists[bcuts + 'Bin' + bcTruthBinInx + histoPrefix + '_' + bkg + denum])
										except:
											print " b-c Bin hists not read in or does not exist for key:" + bcuts + 'Bin' + bcTruthBinInx + histoPrefix + '_' + bkg + denum
											pass
										for plotKey in ['AK4HT', 'Eta']:
											try:
												hists[proc + denum + i +plotKey+ 'Bin' + bcTruthBinInx + bcuts].Add(bkghists[bcuts + 'Bin' + bcTruthBinInx + plotKey+ '_' + lumiStr + '_' + cat + '_' + bkg + denum])
											except:
												print " b-c Bin hists not read in or does not exist for key:" + bcuts + 'Bin' + bcTruthBinInx + plotKey+ '_' + lumiStr + '_' + cat + '_' + bkg + denum
												pass
				...
				if ttHFsf!=1 and 'ttbb' in ttbarGrupList:
					...
					for bcuts in bcutlist:
						...
						Nttbb = hists['ttbb' + denum + i+bcuts].Integral()
						Nttnobb = 0.
						for tt in ttbarGrupList:
							if tt != 'ttbb': Nttnobb += hists[tt + denum + i+bcuts].Integral()
						ttLFsf_ = ttLFsf
						if ttLFsf == -1:
							if Nttnobb != 0:
								...
								ttLFsf_ = 1
						hists['ttbb' + denum + i+bcuts].Scale(ttHFsf)  # Scale iplot B2 B3 B4p
						for tt in list(set(ttbarProcList + ttbarGrupList)):
							if tt != 'ttbb':
								hists[tt + denum + i+bcuts].Scale(ttLFsf_)   # Scale iPlot B2 B3 B4p
						for plotKey in ['AK4HT', 'Eta']:
							hists['ttbb' + denum + i + plotKey+bcuts].Scale(ttHFsf)  # Scale plotKey B2 B3 B4p
							for tt in list(set(ttbarProcList + ttbarGrupList)):
								if tt != 'ttbb':
									hists[tt + denum + i + plotKey+bcuts].Scale(ttLFsf_)  # Scale plotKey B2 B3 B4p

						# print 'ttnobb' + denum + i + bcuts, hists['ttnobb' + denum + i + bcuts].Integral()
						# print 'ttbb' + denum + i + bcuts, hists['ttbb' + denum + i + bcuts].Integral()

						for flavType in ['Bflav', 'topBflav', 'Cflav', 'LiFlav']:
							# if 'ttbb' + i + flavType in hists.keys():
							hists['ttbb' + denum + i + flavType+bcuts].Scale(ttHFsf)  # Scale flavour iplot B2 B3 B4p
							for tt in list(set(ttbarProcList + ttbarGrupList)):
								if tt != 'ttbb':
									# if 'tt' + i + flavType+bcuts in hists.keys():
									hists[tt + denum + i + flavType+bcuts].Scale(ttLFsf_)  # Scale flavour iplot B2 B3 B4p

						for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
							# if 'ttbb' + i + 'Bin' + bcTruthBinInx in hists.keys():
							hists['ttbb' + denum + i + 'Bin' + bcTruthBinInx+bcuts].Scale(ttHFsf) # Scale truth iplot B2 B3 B4p
							for tt in list(set(ttbarProcList + ttbarGrupList)):
								if tt != 'ttbb':
									# if 'tt' + i + 'Bin' + bcTruthBinInx+bcuts in hists.keys():
									hists[tt + denum + i + 'Bin' + bcTruthBinInx+bcuts].Scale(ttLFsf_) # Scale truth iplot B2 B3 B4p
							for plotKey in ['AK4HT', 'Eta']:
								hists['ttbb' + denum + i +plotKey+ 'Bin' + bcTruthBinInx+bcuts].Scale(ttHFsf)  # Scale truth plotKey B2 B3 B4p
								for tt in list(set(ttbarProcList + ttbarGrupList)):
									if tt != 'ttbb':
										hists[tt + denum + i +plotKey+ 'Bin' + bcTruthBinInx+bcuts].Scale(ttLFsf_)  # Scale truth plotKey B2 B3 B4p

						print "                     SCALING 2D tt+bb BY A FACTOR OF", ttHFsf, bcuts, gettime()
						Nttbb = hists['ttbb' + denum + i + "JetPtVsAbsEta"+bcuts].Integral()
						Nttnobb = 0.
						for tt in ttbarGrupList:
							if tt != 'ttbb': Nttnobb += hists[tt + denum + i + "JetPtVsAbsEta"+bcuts].Integral()
						ttLFsf_ = ttLFsf
						if ttLFsf == -1:
							if Nttnobb != 0:
								ttLFsf_ = 1. + (1 - ttHFsf) * (Nttbb / Nttnobb)
								...
								ttLFsf_ = 1
						hists['ttbb' + denum + i + "JetPtVsAbsEta"+bcuts].Scale(ttHFsf) # Scale PT-Eta plot B2 B3 B4p
						for tt in list(set(ttbarProcList + ttbarGrupList)):
							if tt != 'ttbb':
								hists[tt + denum + i + "JetPtVsAbsEta"+bcuts].Scale(ttLFsf_) # Scale PT-Eta plot B2 B3 B4p
					...
					hists['ttbb'+denum+i].Scale(ttHFsf) # Scale iplot
					for tt in list(set(ttbarProcList + ttbarGrupList)):
						if tt != 'ttbb':
							hists[tt+denum + i].Scale(ttLFsf_) # Scale iplot
					for plotKey in ['JetCountInPDG', 'EventCount', 'AK4HT', 'Eta']:
						hists['ttbb' + denum + i+plotKey].Scale(ttHFsf)  # Scale plotKey
						for tt in list(set(ttbarProcList + ttbarGrupList)):
							if tt != 'ttbb':
								hists[tt + denum + i+plotKey].Scale(ttLFsf_)  # Scale plotKey

					for flavType in ['Bflav', 'topBflav', 'Cflav', 'LiFlav']:
						# if 'ttbb'+i+flavType in hists.keys():
						hists['ttbb'+denum+i+flavType].Scale(ttHFsf) # Scale flavour iplot
						for tt in list(set(ttbarProcList+ttbarGrupList)):
							if tt!='ttbb':
								# if 'tt' + i + flavType in hists.keys():
								hists[tt+denum+i+flavType].Scale(ttLFsf_) # Scale flavour iplot

					for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
						# if 'ttbb'+i+'Bin' + bcTruthBinInx in hists.keys():
						hists['ttbb'+denum+i+'Bin' + bcTruthBinInx].Scale(ttHFsf) # Scale truth iplot
						for tt in list(set(ttbarProcList+ttbarGrupList)):
							if tt!='ttbb':
								# if 'tt' + i + 'Bin' + bcTruthBinInx in hists.keys():
								hists[tt+denum+i+'Bin' + bcTruthBinInx].Scale(ttLFsf_) # Scale truth iplot
						for plotKey in ['AK4HT', 'Eta']:
							hists['ttbb' + denum + i+plotKey +'Bin' + bcTruthBinInx].Scale(ttHFsf)  # Scale truth plotKey
							for tt in list(set(ttbarProcList + ttbarGrupList)):
								if tt != 'ttbb':
									hists[tt + denum + i+plotKey +'Bin' + bcTruthBinInx].Scale(ttLFsf_)  # Scale  truth plotKey

					print "                     SCALING 2D tt+bb BY A FACTOR OF", ttHFsf, gettime()
					Nttbb = hists['ttbb' + denum + i + "JetPtVsAbsEta"].Integral()
					Nttnobb = 0.
					for tt in ttbarGrupList:
						if tt != 'ttbb': Nttnobb += hists[tt + denum + i + "JetPtVsAbsEta"].Integral()
					ttLFsf_ = ttLFsf
					if ttLFsf == -1:
						if Nttnobb != 0:
							ttLFsf_ = 1. + (1 - ttHFsf) * (Nttbb / Nttnobb)
							...
							ttLFsf_ = 1
					hists['ttbb' + denum + i + "JetPtVsAbsEta"].Scale(ttHFsf)
					for tt in list(set(ttbarProcList + ttbarGrupList)):
						if tt != 'ttbb':
							hists[tt + denum + i + "JetPtVsAbsEta"].Scale(ttLFsf_)
		...
		for signal in sigList:
			...
			thetaRfileName = outDir+'/templates_'+BRconfStr+'_'+lumiStr+saveKey+'.root'
			print 'thetaRfileName: ' , thetaRfileName
			thetaRfile = TFile(thetaRfileName,'RECREATE')
			for denum in ['Denr', 'Numr']:
				for bcuts in ['B2_', 'B3_', 'B4p_']:
					for cat in catList:
						i = BRconfStr + cat
						if 'B3p' in cat and bcuts=='B2_': continue
						for proc in bkgGrupList:
							hists[proc + denum + i + bcuts].Write()
							for flavType in ['Bflav', 'topBflav', 'Cflav', 'LiFlav']:
								if proc in ttbarGrupList:
									hists[proc + denum + i + flavType+bcuts].Write()
							for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
								if proc in ttbarGrupList:
									hists[proc + denum + i + 'Bin' + bcTruthBinInx+bcuts].Write()
									for plotKey in ['AK4HT', 'Eta']:
										hists[proc + denum + i+plotKey + 'Bin' + bcTruthBinInx + bcuts].Write()

						for proc in bkgGrupList:
							if proc in ttbarGrupList:
								for plotKey in ['JetPtVsAbsEta', 'AK4HT', 'Eta']:
									hists[proc + denum + i + plotKey+bcuts].Write()
								overflow(hists[proc + denum + i + "WeightProd" + bcuts])
								underflow(hists[proc + denum + i + "WeightProd" + bcuts])
								hists[proc + denum + i + "WeightProd" + bcuts].Write()
				for cat in catList:
					i=BRconfStr+cat

					# totBkg_ = sum([hists[proc + denum + i].Integral() for proc in bkgGrupList])
					for proc in bkgGrupList:
						# if proc in bkgGrupList:
						# 		# and hists[proc + denum + i].Integral() / totBkg_ <= removeThreshold:
						# 	print proc + i, 'IS',
						# 	if hists[proc + denum + i].Integral() == 0:
						# 		print 'EMPTY! SKIPPING ...'
						# 	else:
						# 		print '< ' + str(removeThreshold * 100) + '% OF TOTAL BKG! SKIPPING ...'
						# continue
						hists[proc + denum + i].Write()
						for plotKey in ['AK4HT', 'Eta']:
							hists[proc + denum + i + plotKey].Write()

						for flavType in ['Bflav', 'topBflav', 'Cflav', 'LiFlav']:
							if proc in ttbarGrupList :
								# totBkg_ = sum([hists[proc + denum + i + flavType].Integral() for proc in ttbarGrupList])
								# if totBkg_ == 0: continue
								# if hists[proc+denum + i+ flavType].Integral() / totBkg_ <= removeThreshold:
						...
						for bcTruthBinInx in ['1_', '2_', '3_', '4_']:
							if proc in ttbarGrupList:
								hists[proc + denum + i + 'Bin' + bcTruthBinInx].Write()
								for plotKey in ['AK4HT', 'Eta']:
									hists[proc + denum + i + plotKey + 'Bin' + bcTruthBinInx].Write()
